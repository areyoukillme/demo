  ...
  methods: {
    ...
    submitRegister() {
      ...
      this.$post("registerauth", params)
        .then(function(data) {
          if (data.status == 200) {
            self.isregister = "恭喜您，注册成功";
            self.open();
            self.$store.dispatch("login");
            setStorage("islogin", "true");
            setStorage("userId", data.data.userId);
            self.$post("getUser", { userId: getStorage("userId") })
              .then(data => {
                setStorage("face", data.data.image.substring(7));
                setStorage("name", data.data.userName);
                self.$store.dispatch("setuser",data.data.image.substring(7))
                console.log(self.$store.state.userInfo.face)
