	...
	<div class='box f_pr'>
		...
		<div class='headers' :class="isIphoneX?'ipx':''">
			<div class='search_rule'>
				<div class='lis' v-for="(x,index) in rule" @click.stop='exChange(index)' :key="index">
					<span>{{x.name}}</span>
		...
		<div class='goods_list_box' ref="List">
			<div class='goods' v-infinite-scroll="loadMore" infinite-scroll-disabled="loading" infinite-scroll-distance="10">
				<div class='item left f_pr' v-for="(x,index) in SeedList" :key="index">
					<div class='f_pr img overflowimg' @click.stop='goprodetail(x,index)'>
						<img class='fitimg2' :src='x.imageUrl'></img>
						<img class='f_pa self_icon' src='../../assets/self_shop.png'/>
					</div>
					<div class='f_pr goods_bottom'>
						<div class='name ellipsis' :id="x.seedId"> {{x.seedName}}</div>
						<div class='detail'>
							<span class='left'>{{x.spec}}</span>
							<span class='right'>月销{{x.sale}}笔</span>
						</div>
						<div class='price'>
							<span class='price_icon'>￥</span>{{x.price}}
						</div>
					</div>
			...
			<div class="null_list" v-if="allLoaded||SeedList.length==0">没有更多数据了</div>
			<!-- 加载菊花图 -->
	...
	export default {
		...
		data() {
			return {
				...
				sidx: 's.add_time', //查询条件：按新品查询s.add_time,按销量查询s.sale,按价格查询s.price
		...
		methods: {
			...
			touchOnGoods: _.throttle(function(item, $event) {
				this.cartProImg = item.imageUrl;
				this.x = $event.clientX;
				this.y = $event.clientY;
				this.cartItemId = item.seedId;
			...
			async loadMore() {
				...
				let res = await seedslist(this.page, this.limit, this.catId, this.sidx, this.order, this.tokenId);;
				if (res.code == 0) {
					this.showloading = false;
					this.seedList = res.pageUtil.list
					if (res.pageUtil.currPage < res.pageUtil.totalPage) {
						this.page++
						let list = [...this.SeedList, ...this.seedList] //解构拼接数组
						this.SeedList = list;
						this.loading = false;
					} else if (res.pageUtil.currPage == res.pageUtil.totalPage) {
						let list = [...this.SeedList, ...this.seedList] //解构拼接数组
						this.SeedList = list;
						this.allLoaded = true;
						this.loading = true;
			...
			// 获取种子分类
			async categoryseeds() {
				let res = await categoryseeds(this.catType, this.tokenId);
				if (res.code == 0) {
					this.Nav=res.list.concat(res.list);
		...
		mounted() {
			...
			this.$refs.slidebox.addEventListener('scroll', () => {
				this.scrollLeft = this.$refs.slidebox.scrollLeft
			})
		}
	}
</script>
