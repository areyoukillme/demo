			...
			<div class='goods f_pr'>
				<div class='item left f_pr' v-for="(item,index) in fruitList" :key="index" @click.stop='toDetail(item,index)' v-if="shopType==1">
					<div class='f_pr img overflowimg '>
						...
						<img class='f_pa self_icon' src='../../assets/user_shop.png' v-if="item.userId == 1" />
					</div>
				...
				<div class="null_list f_pr" >没有更多数据了</div>
...
<script>
	...
	export default {
		...
		created() {
			...
			let titleName=decodeURI(this.$route.query.titleName)
			console.log(titleName.length)
			if(titleName.length>4){
				this.title=titleName.slice(0,4)+'···的商店'
			}else{
				this.title=titleName+'的商店'
			}
		...
		methods: {
			...
			//获取商品数据
			async getgoodslist() {
				let res = await friendShop(this.page, this.limit, this.sidx, this.order, this.userId, this.tokenId);
				if (res.code == 0) {
					this.fruitList = res.pageUtil.list
					this.isLoad = true
				} else {
					this.$toast({
						message: res.msg,
