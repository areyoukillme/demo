 ...
 * @events  @on-row-click 单击行或者单击操作按钮方法
   ...
   @props   items 显示的结构化数据
            columns 表格列的配置描述sortable:true开启排序功能
...
<template>
    <div :style="{width:tableWidth}" class='autoTbale'>
        <table class="table table-bordered" id='hl-tree-table'>
            ...
            <tbody>
                <tr v-if="initItems.length > 0" v-for="(item,index) in initItems" :key="item.id" v-show="show(item)" :class="{'child-tr':item.parent}">
                    <td v-for="(column,snum) in columns" :key="column.key" :style=tdStyle(column)>
                        ...
                        <div v-if="column.type === 'link'">
                            <a href="javascript:;" @click="RowClick(item,$event,index,column.type)"> {{renderBody(item,column) }} </a>
                        </div>
                        <label @click="toggle(index,item)" v-if="!column.type">
                            <span v-if='snum==iconRow()'>
                                <i v-html='item.spaceHtml'></i>
                                <Icon v-if="item.children&&item.children.length>0" :type="getIconType(item)"></Icon>
                ...
                <tr v-if="initItems.length <= 0">
                  <td :colspan="cloneColumns.length" style="text-align: center;">暂无数据</td>
                </tr>
            </tbody>
        </table>
    </div>
...
<script>
import '@/assets/styles/common/treeTable.less'
export default {
  name: 'treeGrid',
  props: {
    columns: Array,
  ...
  methods: {
    getIconType (item) {
      let typeName = ''
      if (!item.expanded) {
        typeName = 'arrow-right-b'
      } else {
        typeName = 'arrow-down-b'
      }
      return typeName
    },
    // 有无多选框折叠位置优化
