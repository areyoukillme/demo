<template>
  <div class="common-inner merchant-entry-query">
    <!-- 条件选择 -->
    <Row>
      <Col span="24" class="query-inner">
            ...
            <Row>
              <Col span="10">
                <FormItem label="进件状态" class="pay-line-input">
                  <Select v-model="formItem.stateSelect">
                    <Option v-for="s in certificateStatus" :value="s.code" :key="s.code">{{ s.name }}</Option>
                  </Select>
                </FormItem>
              </Col>
            </Row>
    ...
    <!-- 按钮组 -->
    <!-- <Row>
      <Col span="24" class="query-button-box">
        <ButtonGroup>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      certificateStatus: CommonData.certificateStatus,
      columnsName: [
        {
          key: 'index',
          width: 40,
  ...
  methods: {
    getList (currentPage) {
      ...
      options.createTimeBegin = this.getDateFormat(options.createTimeBegin, 'YYYYMMDD', 0, false)
      options.createTimeEnd = this.getDateFormat(options.createTimeEnd, 'YYYYMMDD', 0, false)
