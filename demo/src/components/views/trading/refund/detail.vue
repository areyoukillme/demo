      ...
      <Collapse v-model="defaultShowPanel">
        ...
        <Panel name="3">
          ...
          <div slot="content">
            ...
            <Row>
              <!-- 分页-->
              <Col span="24">
                <Page v-if="listTotal / pageShowSizer  > 1" :total="listTotal" :current="pageCurrent" show-sizer :page-size="pageShowSizer" :page-size-opts="pageSizeOpts" show-total class="common-page" @on-change="getRefundHistory" @on-page-size-change="getPageSizeChange"></Page>
              </Col>
              <!-- 分页 /-->
            </Row>
          </div>
        ...
        <Panel name="4" v-if="this.currType=='edit'">
          ...
          <div slot="content">
            ...
            <Row class="common-bottom">
              <Col span="10">
                <Button size="large"  icon="arrow-return-left" @click="detailReturn">返回</Button>
              </Col>
              <Col span="14" class="common-text-rg">
                <Button type="primary" size="large" :loading="save_loading" icon="ios-search" @click="save" class="pay-check-button">审核通过</Button>
                <Button type="primary" size="large" :loading="unaudit_loading" icon="ios-search" @click="unAudit" class="pay-check-button">审核不通过</Button>
              </Col>
            </Row>
          </div>
        </Panel>
        <div class="common-bottom" v-else>
          <Button type="primary" size="large"  icon="arrow-return-left" @click="detailReturn" class="audit-return-button">返回</Button>
        </div>
      </Collapse>
    </Form>
  </div>
...
export default {
  ...
  data () {
    ...
    return {
      ...
      save_loading: false,
      unaudit_loading: false,
      custCode: this.$route.params.customerCode || -1,
      // 显示第几个折叠面板
      defaultShowPanel: ['1', '2', '3', '4'],
      inputDisabled: true,
      maxLength: 11,
      textareaMax: 200,
      // 初始及最终变量
  ...
  methods: {
    ...
    detailReturn () {
      if (this.currType === 'edit') {
        ...
        this.$router.push({'name': 'trading-refund-query'})
      }
    }
  },
  mounted () {
